function writeAviBoxPlot(h,movieFileName,TimeStep,tzoom)%, scaleLineLength,pixelSize,TimeStep,tzoom, xzoom, yzoom)
...
% if nargin < 7 || isempty(xzoom) || isempty(yzoom)
    xzoom = []; %make empty for full image
    yzoom = []; %make empty for full image
% end

%set limits for ROI
% optoins = plotTracksV1SetOptions('tracks',tracks,'tracksVal',repmat([1:size(tracks,1)]',[1 , size(tracks,2)]));
