function [x, y, Ass_cost] = lap(cc, NONLINK_MARKER, extendedTesting, augmentCC, noLinkCost)
...
% is given by cc(i,j).
%
% SYNOPSIS [x, y, Ass_cost] = lap(cc, NONLINK_MARKER, extendedTesting, augmentCC, noLinkCost)
%
...
%       Ass_cost: the cost of this set of assignments
%
...
% PKR added this assignment cost value to make it compatible with k_best
% algorithm
% determine the cost of this assignment

% PKR - for some reason, row and columns got switched.  however, x
% definitely corresponds to rows to columns (where x(ii) = column and ii is
% the row.
% tic
% mark1 = ismember([colIdx rowIdx],[(1:length(x))' x],'rows');
% toc
