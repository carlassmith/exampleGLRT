...
% if significance_default ~= paramsFilterFits.paramsFilterFits.MaxFPValue
%     [ ~,~,pfa_adj ]=fdr_bh(reshape(rawFitResults.LL(3,:),[prod(size(rawFitResults.LL(3,:))) 1]),paramsFilterFits.MaxFPValue,'dep','no');
mask2=(rawFitResults.PFA <= paramsFilterFits.MaxPFAValue); % & (1-pfa_adj >= paramsFilterFits.MinPValue);
