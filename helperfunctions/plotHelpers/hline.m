...
if length(y)>1  % vector input
    ...
    x=get(gca,'xlim');
    h=plot(x,[y y],linetype,'lineWidth',2.5);
    if ~isempty(label)
        yy=get(gca,'ylim');
        yrange=yy(2)-yy(1);
        yunit=(y-yy(1))/yrange;
        if yunit<0.2
            text(x(1)+0.05*(x(2)-x(1)),y+0.05*yrange,label,'color',get(h,'color'),'fontweight','b')
        else
            text(x(1)+0.05*(x(2)-x(1)),y+0.025*yrange,label,'color',get(h,'color'),'fontweight','b')
        end
    end
